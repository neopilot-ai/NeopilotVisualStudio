...
namespace NeopilotVs.InlineDiff;
...
internal class InlineDiffView
{
    private class InlineDiffTextDataModel : ITextDataModel, IDisposable
    {
        ...
        private void OnContentTypeChanged(object sender, ContentTypeChangedEventArgs e)
        {
            ContentTypeChanged?.Invoke(this,
                                       new TextDataModelContentTypeChangedEventArgs(
    ...
    private static readonly IEnumerable<string> _defaultRoles = new string[] {
        PredefinedTextViewRoles.PrimaryDocument, PredefinedTextViewRoles.Analyzable, Role
    };

    // options for the difference buffer
    private static readonly StringDifferenceOptions _diffBufferOptions =
        new() { DifferenceType = StringDifferenceTypes.Line | StringDifferenceTypes.Word,
                IgnoreTrimWhiteSpace = true };

    public const string Role = "NEOPILOT_INLINE_DIFFERENCE_VIEW";
    ...
    {
        get => _viewer?.ActiveViewType == DifferenceViewType.LeftView ? LeftView : RightView;
    }
    public IVsTextView? ActiveVsView
    {
        get => _viewer?.ActiveViewType == DifferenceViewType.LeftView ? LeftVsView : RightVsView;
    }
    ...
    /// </param>
    public InlineDiffView(IWpfTextView hostView, IProjectionBuffer leftProjection,
                          ITextBuffer leftBuffer, IProjectionBuffer rightProjection,
    ...
    public void Dispose()
    {
        ...
        if (RightTextLines is IVsPersistDocData vsPersistDocData2) vsPersistDocData2.Close();
    ...
    private void CreateTextViewHostCallback(IDifferenceTextViewModel textViewModel,
                                            ITextViewRoleSet roles, IEditorOptions options,
    ...
    /// <summary>
    /// Set the role and text view model for the code window created by <see
    /// cref="CreateTextViewHostCallback"/>.
    /// </summary>
    /// <param name="codeWindowData"></param>
    /// <param name="textViewModel"></param>
    /// <param name="diffRoles"></param>
    private void SetRolesAndModel(IVsUserData codeWindowData,
                                  IDifferenceTextViewModel textViewModel,
                                  ITextViewRoleSet diffRoles)
    {
        ...
        IEnumerable<string> enumerable = _defaultRoles.Concat<string>(
            MefProvider.Instance.TextEditorFactoryService.DefaultRoles);
        if (diffRoles != null)
            enumerable = enumerable.Concat(
                MefProvider.Instance.TextEditorFactoryService.CreateTextViewRoleSet(diffRoles));

        // set the roles for this code window
        string vtData =
            MefProvider.Instance.TextEditorFactoryService.CreateTextViewRoleSet(enumerable)
                .ToString();
    ...
    private void InitializeView(IWpfTextView view, IWpfTextViewHost host)
    {
        ...
        // disable scrollbars
        // view.Options.SetOptionValue(DefaultTextViewHostOptions.HorizontalScrollBarId        ,
        // false);
        ...
        // view.Options.SetOptionValue(DefaultTextViewHostOptions.LineEndingMarginOptionId     ,
        // false);
        ...
        // disable the controls in the horizontal scroll bar area (the PredefinedMarginNames.Bottom
        // margin)
        view.Options.SetOptionValue(DefaultTextViewHostOptions.EnableFileHealthIndicatorOptionId,
                                    false);
        view.Options.SetOptionValue(DefaultTextViewHostOptions.EditingStateMarginOptionId, false);
        view.Options.SetOptionValue(DefaultTextViewHostOptions.IndentationCharacterMarginOptionId,
                                    false);
        ...
        // host.GetTextViewMargin("deltadifferenceViewerOverview").VisualElement.Visibility =
        ...
        host.GetTextViewMargin("deltaDifferenceAdornmentMargin").VisualElement.Visibility =
            Visibility.Collapsed;

        // intellicode
        // Microsoft.VisualStudio.IntelliCode.WholeLineCompletion.UI.LineCompletionMenuEditorMargin.MarginName
        // host.GetTextViewMargin("LineCompletionMenuEditorMargin").VisualElement.Visibility =
        // Visibility.Collapsed;
        // host.GetTextViewMargin(PredefinedMarginNames.LineEndingMargin).VisualElement.Visibility =
        // Visibility.Collapsed;
        ...
        // var edges = host.GetType().GetField("_edges", BindingFlags.NonPublic |
        // BindingFlags.Instance).GetValue(host) as List<IWpfTextViewMargin>; foreach (var edge in
        // edges)
        //{
    ...
    /// <summary>
    /// Get the max height of both text views, not including its adornments like "peek definition"
    /// window and the horizontal scroll bar.
    /// </summary>
    /// <returns></returns>
    public double ContentHeight()
    {
        ...
        Difference difference = _viewer.DifferenceBuffer.CurrentSnapshotDifference?.LineDifferences
        ...
        textView.DisplayTextLineContainingBufferPosition(
            new SnapshotPoint(textView.TextSnapshot, 0),
            0.0,
        ...
        double leftHeight =
            textView.TextViewLines[textView.TextViewLines.Count - 1].Bottom - textView.ViewportTop;
        double rightHeight = textView2.TextViewLines[textView2.TextViewLines.Count - 1].Bottom -
                             textView2.ViewportTop;
    ...
    private void EnsureContentsVisible()
    {
        _viewer.RightView.QueuePostLayoutAction(delegate {
            if (_viewer.RightView.TextViewLines[0].Start.Position != 0 ||
                _viewer.RightView.TextViewLines[0].Top < _viewer.RightView.ViewportTop)
                _viewer.RightView.DisplayTextLineContainingBufferPosition(
                    new SnapshotPoint(_viewer.RightView.TextSnapshot, 0),
                    0.0,
                    ViewRelativePosition.Top);
        });

        _viewer.LeftView.QueuePostLayoutAction(delegate {
            if (_viewer.LeftView.TextViewLines[0].Start.Position != 0 ||
                _viewer.LeftView.TextViewLines[0].Top < _viewer.LeftView.ViewportTop)
                _viewer.LeftView.DisplayTextLineContainingBufferPosition(
                    new SnapshotPoint(_viewer.LeftView.TextSnapshot, 0),
                    0.0,
                    ViewRelativePosition.Top);
        });
    }
    ...
    private void RecalculateSize()
    {
        ...
        // the scroll bar area
        double? leftBottomHeight =
            LeftHost.GetTextViewMargin(PredefinedMarginNames.Bottom)?.VisualElement.ActualHeight;
        double? rightBottomHeight =
            RightHost.GetTextViewMargin(PredefinedMarginNames.Bottom).VisualElement.ActualHeight;
        if (leftBottomHeight != null && rightBottomHeight != null)
        {
            ...
            // try to make them the same height, because the right one will have intellicode button
            // and it can't be disabled
            var leftScroll =
                LeftHost.GetTextViewMargin(PredefinedMarginNames.HorizontalScrollBarContainer);
            var rightScroll =
                RightHost.GetTextViewMargin(PredefinedMarginNames.HorizontalScrollBarContainer);
    ...
    /// <summary>
    /// Block the mouse wheel input to the diff view and send it to the hostview instead. Applied
    /// for both inner text views.
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void DiffView_OnPreviewMouseWheel(object sender,
                                              System.Windows.Input.MouseWheelEventArgs e)
    {
        ...
        _hostView.ViewScroller.ScrollViewportVerticallyByLines(
            e.Delta > 0 ? ScrollDirection.Up : ScrollDirection.Down, 3);
        e.Handled = true;
    }
    ...
    /// view and disable for others.
    /// </summary>
    /// <param name="view"></param>
    private void ShowSelectedLineForView(IWpfTextView view)
    {
        LeftView?.Options.SetOptionValue(DefaultWpfViewOptions.EnableHighlightCurrentLineId,
                                         (view == LeftView));
        RightView?.Options.SetOptionValue(DefaultWpfViewOptions.EnableHighlightCurrentLineId,
                                          (view == RightView));
        _hostView?.Options.SetOptionValue(DefaultWpfViewOptions.EnableHighlightCurrentLineId,
                                          (view == _hostView));
    }

    private void RightView_OnLostFocus(object sender, RoutedEventArgs e)
    {
        if (!LeftView.VisualElement.IsFocused) ShowSelectedLineForView(_hostView);
    }

    private void LeftView_OnLostFocus(object sender, RoutedEventArgs e)
    {
        if (!RightView.VisualElement.IsFocused) ShowSelectedLineForView(_hostView);
    }
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void DiffBuffer_SnapshotDifferenceChanged(object sender,
    ...
    private void RightView_OnCaretPositionChanged(object sender, CaretPositionChangedEventArgs e)
    {
        ...
        // ITextViewLine line =
        // LeftView.TextViewLines.GetTextViewLineContainingBufferPosition(point); if (line != null)
        // LeftView.Caret.MoveTo(line);
    }
    ...
    private void LeftView_OnCaretPositionChanged(object sender, CaretPositionChangedEventArgs e)
    {
        ...
        // ITextViewLine line =
        // RightView.TextViewLines.GetTextViewLineContainingBufferPosition(point); if (line != null)
        // RightView.Caret.MoveTo(line);
    }
    ...
    private IVsTextLines CreateVsTextLines(InlineDiffTextDataModel dataModel)
    {
        ...
        if (_vsTextBufferAdapterCtor == null)
        {
            ...
            Assembly assembly =
    MefProvider.Instance.EditorAdaptersFactoryService.GetType().Assembly;

            // Get the `VsTextBufferAdapter` constructor
            Type typeVsTextBufferAdapter =
    assembly?.GetType("Microsoft.VisualStudio.Editor.Implementation.VsTextBufferAdapter");
            _vsTextBufferAdapterCtor = typeVsTextBufferAdapter?.GetConstructor([]);

            Assumes.True(_vsTextBufferAdapterCtor != null,
                $"InlineDiffView.CreateTextLines: Failed to get the constructor for
    VsTextBufferAdapter. " +
        ...
        type.GetField("_documentTextBuffer", privateFlag).SetValue(vsTextLines,
    dataModel.DataBuffer); type.GetField("_dataTextBuffer"    , privateFlag).SetValue(vsTextLines,
    dataModel.DataBuffer);

        // we MUST set the _serviceProvider before calling SetSite
        type.GetField("_serviceProvider"   , privateFlag).SetValue(vsTextLines,
    MefProvider.Instance.OleServiceProvider); type.GetMethod("SetSite"           ,
    publicFlag).Invoke(vsTextLines,  [MefProvider.Instance.OleServiceProvider]);
        ...
        baseType.GetMethod("OnTextBufferInitialized"     , privateFlag).Invoke(vsTextLines,
    _twoNulls);
