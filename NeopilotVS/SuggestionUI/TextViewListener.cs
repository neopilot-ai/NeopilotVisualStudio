...
internal class NeopilotCompletionHandler : IOleCommandTarget, IDisposable
{
    private readonly NeopilotVSPackage package;
    ...
    public static async Task<Command> GetCommandsAsync(String name)
    {
        ...
        {
            NeopilotVSPackage.Instance.LogAsync(ex.ToString());
        }
    ...
    private void OnSuggestionAccepted(String proposalId)
    {
        ...
        ThreadHelper.JoinableTaskFactory
            .RunAsync(async delegate {
                await NeopilotVSPackage.Instance.LogAsync($"Accepted completion {proposalId}");
                await NeopilotVSPackage.Instance.LanguageServer.AcceptCompletionAsync(proposalId);
            })
            .FireAndForget(true);
    }
    ...
    internal NeopilotCompletionHandler(IVsTextView textViewAdapter, ITextView view,
                                      TextViewListener provider)
    {
        try
        {
            NeopilotVSPackage.EnsurePackageLoaded();
            ...
            if (_document != null)
            {
                NeopilotVSPackage.Instance.LogAsync("NeopilotCompletionHandler filepath = " +
                                                   _document.FilePath);
            ...
            _ = Task.Run(() =>
                         {
                             try
                             {
                                 CompleteSuggestionCommand =
                                     GetCommandsAsync("NeopilotAcceptCompletion").Result;
                             }
                             catch (Exception e)
                             {
                                 Debug.Write(e);
    ...
    private void CaretUpdate(object sender, CaretPositionChangedEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ThreadHelper.JoinableTaskFactory
                .RunAsync(
                    async delegate { await NeopilotVSPackage.Instance.LogAsync(ex.ToString()); })
                .FireAndForget(true);
        }
    }
    ...
    private void RefreshLanguage()
    {
        ...
        {
            ThreadHelper.JoinableTaskFactory.RunAsync(() => NeopilotVSPackage.Instance.LogAsync($"RefreshLanguage Error: {ex}")).FireAndForget();
        }
    }
    ...
    void ShowIntellicodeMsg()
    {
        if (IsIntellicodeEnabled())
        {
            VsShellUtilities.ShowMessageBox(
                this.package,
                "Please disable IntelliCode to use Neopilot. You can access Intellicode settings " +
...
internal class TextViewListener : IVsTextViewCreationListener
{
    ...
    public void VsTextViewCreated(IVsTextView textViewAdapter)
    {
        ...
        Func<NeopilotCompletionHandler> createCommandHandler = delegate()
        {
            return new NeopilotCompletionHandler(textViewAdapter, textView, this);
        };
        textView.TextBuffer.Properties.GetOrCreateSingletonProperty<NeopilotCompletionHandler>(
            typeof(NeopilotCompletionHandler), createCommandHandler);
    }
}
}
