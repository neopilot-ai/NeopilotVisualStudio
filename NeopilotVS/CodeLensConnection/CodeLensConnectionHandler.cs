    ...
    public static async Task AcceptCodeLensConnections()
    {
        try
        {
            while (true)
            {
                try
                {
                    var stream = new NamedPipeServerStream(PipeName.Get(Process.GetCurrentProcess().Id),
                                                           PipeDirection.InOut,
                ...
                {
                    NeopilotVSPackage.Instance.LogAsync($"AcceptCodeLensConnections Error: {ex}");
                    // Wait a bit before retrying to avoid hot loops in case of persistent errors
        ...
        {
            NeopilotVSPackage.Instance.LogAsync($"AcceptCodeLensConnections Fatal Error: {ex}");
        }
        ...
        static async Task HandleConnection(NamedPipeServerStream stream)
        {
            ...
            {
                NeopilotVSPackage.Instance.LogAsync("Handle Connection Error");
            }
        }
    }
