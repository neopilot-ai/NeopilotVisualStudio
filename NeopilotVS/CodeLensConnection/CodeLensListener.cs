...
public class CodeLensListener : ICodeLensCallbackListener, ICodeLensListener
{
    ...
    public bool
    IsNeopilotCodeLensActive() => NeopilotVSPackage.Instance != null
                                 && NeopilotVSPackage.Instance.SettingsPage.EnableCodeLens;
                                                     ...
                                                     CancellationToken ct)
    {
        try
        {
            ...
            NeopilotCompletionHandler handler;
            if (props.ContainsProperty(key))
            {
                handler = props.GetProperty<NeopilotCompletionHandler>(key);
            }
            else
            {
                handler = null;
