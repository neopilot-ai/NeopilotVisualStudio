...
{
    // Pipe needs to be scoped by PID so multiple VS instances don't compete for connecting
    // CodeLenses.
    public static string Get(int pid) => $@"neopilot\{pid}";
}
...
public interface ICodeLensListener
{
    ...
    bool IsNeopilotCodeLensActive();
    Task<FunctionInfo> LoadInstructions(Guid dataPointId, Guid projGuid, string filePath,
