...
namespace NeopilotVS.QuickInfo;

internal sealed class NeopilotAsyncQuickInfoSource
(ITextBuffer textBuffer)
    : PropertyOwnerExtension<ITextBuffer, NeopilotAsyncQuickInfoSource>(textBuffer),
      IAsyncQuickInfoSource, IDisposable
{
                                                           ...
                                                           CancellationToken cancellationToken)
    {
        ...
        Assumes.True(_tagAggregator != null,
                     "Neopilot Quick Info Source couldn't create a tag aggregator for error tags");
        ...
        {
            Debug.Fail("The Neopilot Quick Info Source is being called when it shouldn't be.");
            return null;
        }
        ...
        if (appToSpan != null && problemMessage.Length > 0)
        {
            var hyperLink = ClassifiedTextElement.CreateHyperlink(
                "Neopilot: Explain Problem",
                "Ask Neopilot to explain the problem",
                () =>
                {
                    ThreadHelper.JoinableTaskFactory
                        .RunAsync(async delegate {
                            // TODO: Has the package been loaded at this point?
                            await NeopilotVSPackage.Instance.LanguageServer.Controller
                                .ExplainProblemAsync(
    ...
    private async Task GetTagAggregatorAsync(ITextView textView)
    {
        ...
        else if (_tagAggregatorTextView != textView)
        {
            throw new ArgumentException(
                "The Neopilot Quick Info Source cannot be shared between TextViews.");
        }
    }
}

[Export(typeof(IAsyncQuickInfoSourceProvider))]
[Name("Neopilot Quick Info Provider")]
[ContentType("any")]
...
internal sealed class AsyncQuickInfoSourceProvider : IAsyncQuickInfoSourceProvider
{
    ...
    {
        return NeopilotAsyncQuickInfoSource.GetOrCreate(
            textBuffer, () => new NeopilotAsyncQuickInfoSource(textBuffer));
    }
}
