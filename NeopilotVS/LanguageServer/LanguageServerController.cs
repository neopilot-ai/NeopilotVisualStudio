...
{
    readonly NeopilotVSPackage Package;
    ...
    private void InsertText(string text)
    {
        ThreadHelper.JoinableTaskFactory
            .RunAsync(async delegate {
                ...
                DocumentView? docView = await Utilities.ViewUtils.GetActiveDocumentViewAsync();
    ...
    private void ApplyDiff(string filePath, int start_line, int end_line, string replacement)
    {
        ThreadHelper.JoinableTaskFactory
            .RunAsync(async delegate {
                ...
                // we'd have to wait for the `NeopilotInlineDiffViewProvider` to run
...
internal static class WebChatServer
{
    internal static WebServerRequest NewRequest()
    {
        ...
        WebServerRequest request = new() { get_chat_message_request = new() {
            ...
            metadata = NeopilotVSPackage.Instance?.LanguageServer.GetMetadata(),
            prompt = ""
        } };
    ...
    internal static bool Send(this WebServerRequest request, WebSocket ws)
    {
        ...
        {
            NeopilotVSPackage.Instance.Log("Language Server Controller: Unable to send the " +
