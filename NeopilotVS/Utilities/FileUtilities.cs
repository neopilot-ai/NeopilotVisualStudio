    ...
    internal static void DeleteSafe(string path)
    {
        ...
        catch (Exception ex)
        {
            if (ex is UnauthorizedAccessException || ex is IOException)
            {
                ...
                {
                    NeopilotVSPackage.Instance?.Log(
                        $"Failed to move the file why trying to delete it, exception: {ex2}");
                    VS.MessageBox.ShowError(
                        $"Neopilot: Failed to move the file why trying to delete it: {path}",
                        "Please see the output windows for more details");
                }
            }
            else
            {
                NeopilotVSPackage.Instance?.Log($"Failed to delete file, exception: {ex}");
                VS.MessageBox.ShowError($"Neopilot: Failed to delete file: {path}",
                                        "Please see the output windows for more details");
            }
        }
    }
