...
internal static class CodeAnalyzer
{
    ...
    public static Span GetBlockSpan(ITextView view, int position, out IStructureTag? outtag)
    {
        ...
        ITagAggregator<IStructureTag> tagAggregator =
            IntellisenseUtilities.GetTagAggregator<IStructureTag>(view);
        ...
        foreach (var mappingTag in mappingTags)
        {
            ...
            else
            {
                ...
                if (tag.HeaderSpan?.Start > position || tag.OutliningSpan?.End <= position)
                    continue;
        ...
        where T : class
    {
        ...
        if (languageService != null) _languageServices.Add(languageServiceId, languageService);

        return languageService as T;
    }

    public static T? GetLanguageService<T>(Guid languageServiceId)
        where T : class
    {
        if (_languageServices.TryGetValue(languageServiceId, out var languageService))
            return languageService as T;
                                                                   ...
                                                                   int column)
    {
        ...
        if (ErrorHandler.Failed(vsTextLines.GetLanguageServiceID(out var languageServiceID)))
            return null;

        IVsLanguageBlock? vsLanguageBlock =
            await GetLanguageServiceAsync<IVsLanguageBlock>(languageServiceID);
        if (vsLanguageBlock == null) return null;

        TextSpan[] spans = [new TextSpan()];

        if (ErrorHandler.Failed(vsLanguageBlock.GetCurrentBlock(
                vsTextLines, line, column, spans, out string desc, out int avail)))
            return null;
    ...
    public static FunctionBlock? GetFunctionBlock(IWpfTextView view, int line, int column)
    {
        ...
        return ThreadHelper.JoinableTaskFactory.Run(
            async delegate { return await GetFunctionBlockAsync(view, line, column); });
    }
}
